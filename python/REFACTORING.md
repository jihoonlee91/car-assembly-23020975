...
- [ ] **0. 계획 문서 작성** (본 문서) — 실제 구현 전 계획을 먼저 문서화하고 커밋
- [ ] **1. 모델/규칙 계층 작성** — `models.py`, `rules.py` 추가. 아직 기존 `assemble.py`는 건드리지 않음. `tests/test_rules.py`로 5개 규칙 + PASS 케이스를 pytest로 검증 (순수 로직이라 UI 없이 먼저 테스트 가능)
